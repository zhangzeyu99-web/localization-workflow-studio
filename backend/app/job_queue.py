...
def enqueue_job(
    *,
    ...
    with db.connect() as conn:
        ...
        db.require_project_active(conn, project_id)
        ...
        if row is not None:
            db.require_project_active(conn, str(row["project_id"] or ""))
    if row is None:
...
def activate_job(job_id: str, *, autostart: bool = True) -> JobRecord:
    ...
    with db.connect() as conn:
        ...
        existing = conn.execute(
            "SELECT project_id FROM job_queue WHERE job_id = ?",
            (job_id,),
        ).fetchone()
        if existing is None:
            raise KeyError(f"job not found: {job_id}")
        db.require_project_active(conn, str(existing["project_id"] or ""))
        conn.execute(
...
def claim_next_job(lane: str) -> JobRecord | None:
    ...
    with db.connect() as conn:
        ...
        candidate = conn.execute(
            """
            SELECT job_queue.id FROM job_queue
            JOIN projects ON projects.id = job_queue.project_id
            WHERE job_queue.lane = ?
              AND job_queue.status = 'queued'
              AND projects.lifecycle_state = 'active'
            ORDER BY job_queue.queued_at ASC, job_queue.id ASC
            LIMIT 1
            """,
            (lane,),
        ).fetchone()
...
def recover_interrupted_jobs() -> list[JobRecord]:
    ...
    for row in rows:
        result = _record(row)
        canceled = bool(result.get("cancel_requested"))
        result["status"] = "canceled" if canceled else "interrupted"
        if canceled:
            result["canceled_at"] = result.get("canceled_at") or timestamp

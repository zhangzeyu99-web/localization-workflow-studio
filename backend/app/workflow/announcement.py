...
def cancel_announcement_task(task_id: str, expected_statuses: list[str] | None = None) -> dict[str, Any]:
    task = db.cancel_announcement_task(task_id, db.now_iso(), expected_statuses)
    return {"task": _hydrate_announcement_task(task)}
...
def deliver_announcement_task(task_id: str, request: Any) -> dict[str, Any]:
    ...
    if existing_artifact and not force:
        ...
        task = db.update_announcement_task(
            task_id,
            status="delivered",
            current_step=ANNOUNCEMENT_STEP["deliver"],
            metadata=metadata,
            allow_terminal_update=task.get("status") == "delivered",
        )
        return {

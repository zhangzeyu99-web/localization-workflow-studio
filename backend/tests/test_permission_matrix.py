...
def test_member_queue_views_and_cancel_are_limited_to_member_projects(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    test_app = _required_app(monkeypatch)
    with TestClient(test_app) as client:
        _bootstrap_admin(client)
        member = _create_user(client, "queue-member", "member")
        visible_project = client.post(
            "/api/projects",
            json={"name": "Visible Queue Project", "type": "QA"},
        ).json()
        hidden_project = client.post(
            "/api/projects",
            json={"name": "Hidden Queue Project", "type": "QA"},
        ).json()
        db.add_project_member(visible_project["id"], member["id"], added_by="root-admin")

        visible_run = db.insert_run(
            visible_project["id"],
            "translation",
            metadata={"task_origin": "translation_run"},
        )
        hidden_run = db.insert_run(
            hidden_project["id"],
            "translation",
            metadata={"task_origin": "translation_run"},
        )
        visible_job_id = f"run:{visible_run['id']}"
        hidden_job_id = f"model-fix:{hidden_run['id']}"
        job_queue.enqueue_job(
            job_id=visible_job_id,
            lane="language_table",
            job_kind="translation",
            project_id=visible_project["id"],
            target_id=visible_run["id"],
            autostart=False,
        )
        job_queue.enqueue_job(
            ...
            target_id=hidden_run["id"],
            autostart=False,
        )
        assert job_queue.claim_next_job("language_table") is not None
        assert job_queue.claim_next_job("quick_announcement") is not None

        client.post("/api/auth/logout")
        _login(client, "queue-member")

        active = client.get("/api/system/active-jobs")
        queues = client.get("/api/system/job-queues")
        hidden_cancel = client.post(f"/api/system/job-queues/{hidden_job_id}/cancel")
        visible_cancel = client.post(f"/api/system/job-queues/{visible_job_id}/cancel")

    assert active.status_code == 200, active.text
    assert {entry["job_id"] for entry in active.json()} == {visible_job_id}
    assert queues.status_code == 200, queues.text
    visible_queue_job_ids = {
        entry["job_id"]
        for lane in queues.json()["lanes"]
        for entry in [lane["running"], *lane["queued"]]
        if entry is not None
    }
    assert visible_queue_job_ids == {visible_job_id}
    assert hidden_cancel.status_code == 404, hidden_cancel.text
    assert hidden_cancel.json() == {"detail": "project not found"}
    assert job_queue.get_job(hidden_job_id)["cancel_requested"] is False
    assert visible_cancel.status_code == 200, visible_cancel.text
    assert job_queue.get_job(visible_job_id)["cancel_requested"] is True

...
def test_business_lane_classification_keeps_every_quick_task_stage_on_the_quick_lane() -> None:
    ...
    assert background_jobs.lane_for_run(quick_run, "model_fix") == "quick_announcement"
...
def test_business_reconcile_replays_queued_announcement_cancel_after_crash() -> None:
    background_jobs = importlib.import_module("app.background_jobs")
    queue = _queue()
    project = db.insert_project("restart queued announcement cancel", "QA", "")
    task = db.insert_announcement_task(
        project["id"],
        {"title": "cancel replay", "selected_languages": ["en"], "status": "queued", "current_step": 7},
    )
    db.upsert_announcement_task_language(
        task["id"],
        project["id"],
    ...
    canceled = queue.cancel_job(job_id, canceled_by="Alice")
    ...
    stored = db.get_announcement_task(task["id"])
    assert queue.get_job(job_id)["status"] == "canceled"
    assert stored["status"] == "prepared"
    assert stored["metadata"]["canceled_by"] == "Alice"
    assert stored["metadata"]["canceled_at"] == canceled["canceled_at"]
    assert {item["status"] for item in stored["languages"]} == {"prepared"}
    assert summary["recovered_canceled_jobs"] == 1

    snapshot = db.get_announcement_task(task["id"])
    background_jobs.reconcile_startup([])
    assert db.get_announcement_task(task["id"]) == snapshot


def test_business_reconcile_preserves_translate_cancel_semantics_after_running_crash() -> None:
    background_jobs = importlib.import_module("app.background_jobs")
    queue = _queue()
    project = db.insert_project("restart running announcement translate cancel", "QA", "")
    task = db.insert_announcement_task(
        project["id"],
        {"title": "running translate cancel", "selected_languages": ["en"], "status": "running", "current_step": 7},
    )
...
def test_business_reconcile_finishes_canceled_announcement_child_after_parent_write_crash() -> None:
    background_jobs = importlib.import_module("app.background_jobs")
    queue = _queue()
    project = db.insert_project("restart partial announcement cancel", "QA", "")
    task = db.insert_announcement_task(
        project["id"],
        {"title": "partial cancel", "selected_languages": ["en"], "status": "queued", "current_step": 7},
    )
    db.upsert_announcement_task_language(task["id"], project["id"], "en", status="running", current_step=7)
    job_id = f"announcement:{task['id']}"
    _enqueue(
    ...
    db.update_announcement_task(task["id"], status="prepared", current_step=7, metadata=audit)
    ...
    assert stored["status"] == "prepared"
    assert {item["status"] for item in stored["languages"]} == {"prepared"}
    assert stored["languages"][0]["metadata"]["canceled_at"] == canceled["canceled_at"]
    assert summary["recovered_canceled_jobs"] == 1


def test_business_reconcile_preserves_task_level_announcement_cancel_after_crash() -> None:
    background_jobs = importlib.import_module("app.background_jobs")
    queue = _queue()
    project = db.insert_project("restart whole announcement cancel", "QA", "")
    task = db.insert_announcement_task(
        project["id"],
        {"title": "whole task cancel", "selected_languages": ["en"], "status": "running", "current_step": 7},
    )
    job_id = f"announcement:{task['id']}"
    _enqueue(
        job_id,
    ...
    db.cancel_announcement_task(
        task["id"],
        canceled_at,
        audit_patch={"cancel_scope": "task", "canceled_by": "Alice"},
    )
    ...
    assert stored["metadata"]["cancel_scope"] == "task"
    assert {item["status"] for item in stored["languages"]} == {"canceled"}


def test_business_reconcile_replays_interrupted_announcement_after_second_crash() -> None:
    background_jobs = importlib.import_module("app.background_jobs")
    queue = _queue()
    project = db.insert_project("restart interrupted announcement twice", "QA", "")
    task = db.insert_announcement_task(
        project["id"],
        {"title": "interrupt replay", "selected_languages": ["en"], "status": "running", "current_step": 7},
    )
    db.upsert_announcement_task_language(
        task["id"],
        project["id"],
    ...
    assert [row["job_id"] for row in queue.recover_interrupted_jobs()] == [job_id]
    assert queue.get_job(job_id)["status"] == "interrupted"
    ...
    assert stored["metadata"]["reason"] == "service_restart_continue"
    assert {item["status"] for item in stored["languages"]} == {"prepared"}
    assert summary["interrupted_announcements"] == 1

    interrupted_at = stored["metadata"]["interrupted_at"]
    background_jobs.reconcile_startup([])
    assert db.get_announcement_task(task["id"])["metadata"]["interrupted_at"] == interrupted_at


def test_business_reconcile_finishes_interrupted_announcement_child_after_parent_write_crash() -> None:
    background_jobs = importlib.import_module("app.background_jobs")
    queue = _queue()
    project = db.insert_project("restart partial announcement interruption", "QA", "")
    task = db.insert_announcement_task(
        project["id"],
        {"title": "partial interruption", "selected_languages": ["en"], "status": "running", "current_step": 7},
    )
    db.upsert_announcement_task_language(task["id"], project["id"], "en", status="running", current_step=7)
    job_id = f"announcement:{task['id']}"
    _enqueue(
    ...
    db.update_announcement_task(
        ...
        status="needs_input",
        current_step=7,
        metadata={"reason": "service_restart_continue", "interrupted_at": interrupted_at},
    )
    ...
    stored = db.get_announcement_task(task["id"])
    assert stored["status"] == "needs_input"
    assert stored["metadata"]["interrupted_at"] == interrupted_at
    assert {item["status"] for item in stored["languages"]} == {"prepared"}
    assert summary["interrupted_announcements"] == 1


def test_business_reconcile_recovers_legacy_announcement_without_queue_row() -> None:
    background_jobs = importlib.import_module("app.background_jobs")
    project = db.insert_project("legacy announcement without persistent queue", "QA", "")
    task = db.insert_announcement_task(
        project["id"],
        {"title": "legacy recovery", "selected_languages": ["en"], "status": "queued", "current_step": 7},
    )
    db.upsert_announcement_task_language(
        task["id"],
        project["id"],

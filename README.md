...
- 工作台是本地桌面 Web 应用：后端默认只应绑定本机地址。v1.7.0 同时提供默认使用 `cloud/required` 的有账号包，以及锁定为 `cloud/off` 的内网无账号包；无账号包仅适用于已有网络隔离的可信公司内网，不得暴露到公网。本地默认仍然免登录，行为不变。
...
当前版本：`1.7.0`。源码支持本地 `local/off`、内网无账号 `cloud/off` 和线上有账号 `cloud/required`；本次同时提供默认使用 `cloud/required` 的有账号包和锁定为 `cloud/off` 的无账号包。

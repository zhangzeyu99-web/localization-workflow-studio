...
def classify_name_type(category: str) -> str:
    ...
    if not text:
        return ""
    if any(marker in text for marker in _SKILL_EXCLUDES | _LOCATION_EXCLUDES):
        return ""
...
def resolve_name_type(source: str, term_lookup: dict[str, Any] | None) -> str:
    ...
    if isinstance(entry, str):
        return entry if entry in {SKILL_NAME_TYPE, LOCATION_NAME_TYPE} else ""
    if not isinstance(entry, dict):
        return ""
    explicit = str(entry.get("name_type", "")).strip()
    if explicit in {SKILL_NAME_TYPE, LOCATION_NAME_TYPE}:
        return explicit
...
def find_name_collisions(
    ...
    for row in rows:
        ...
        if name_type not in {SKILL_NAME_TYPE, LOCATION_NAME_TYPE}:
            continue

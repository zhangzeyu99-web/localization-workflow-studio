...
默认输出名为 `有账号-v1.7.2.zip`，包内根目录为 `localization-workflow-studio-v1.7.2-g<sha12>-universal`；无账号输出名必须为 `无账号-v1.7.2.zip`，包内根目录为 `localization-workflow-studio-v1.7.2-g<sha12>-cloud-off`。每个 ZIP 旁边必须有同名 `.sha256`，包内必须包含 `PACKAGE_MANIFEST.json`、`SHA256SUMS.txt` 和 `DEPLOY_README.zh-CN.md`。
...
使用 `无账号-v1.7.2.zip` 内已锁定的 `deploy/lws.env.example` 安装服务器环境。部署检查不传登录凭据：

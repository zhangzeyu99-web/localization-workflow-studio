...
function stripTermNoise(value: string): string {
  return value
    .replace(/术语保持一致[：:][^。；;]*[。；;]?/g, '')
    .replace(/[^。；;]*核心术语[^。；;]*[。；;]?/g, '')
    .replace(/[^。；;]*术语优先一致性[^。；;]*[。；;]?/g, '')
...
function generatedChinesePrompt(project: Project, code: LanguageCode): string {
  ...
  const focus = stripTermNoise(firstProfileText(profile.display_focus, profileTableValue(source, ['重点注意', '注意事项'])))
  return [
    ...
    '\u672f\u8bed\u8bd1\u6cd5\u4ee5\u672c\u6b21\u4efb\u52a1\u968f\u9644\u672f\u8bed\u8868\u3001\u884c\u7ea7\u672f\u8bed\u547d\u4e2d\u548c\u8bd1\u6587\u5f52\u6863\u547d\u4e2d\u4e3a\u51c6\u3002',
...
function looksLikeExecutionPrompt(value: string): boolean {
  ...
  if (/核心术语|术语保持一致|项目术语以术语表为准|已有.+译文代表项目历史用法/.test(value)) return true
  if (/\{\s*["']?source["']?\s*:|\{\s*source\s*:|["']target["']\s*:/.test(value)) return true

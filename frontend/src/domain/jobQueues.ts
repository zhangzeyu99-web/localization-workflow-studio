...
function runMatchesTranslationTask(run: Run, translationTaskId: string): boolean {
  return String(run.metadata?.translation_task_id || '') === translationTaskId
}
...
export function formalWorkflowQueueJob(
  ...
  const jobs = [...(languageLane?.running ? [languageLane.running] : []), ...(languageLane?.queued || [])]
    .filter((job) => (
      ...
      && String(job.translation_task_id || '') === translationTaskId
    ))
  ...
  const candidates = [currentRun, ...(project.runs || [])]
    ...
    .filter((run) => runMatchesTranslationTask(run, translationTaskId))

...
`, badWorkbook])
  ...
  await expect(page.getByTestId('delivery-problem-warning')).toContainText('待生成带问题交付')
  await expect(page.getByTestId('delivery-problem-warning')).toContainText('生成交付时会附带问题摘要')
  await expect(page.locator('.delivery-card .warn-line')).toHaveCount(0)
  await page.locator('[data-testid^="delivery-generate-"]').first().click()
  ...
  await expect(inlineStatus(page, '手工修复已重新 QA：已通过')).toBeVisible({ timeout: 60000 })

...
test('user can complete the EN localization workflow from project tabs', async ({ page, request }) => {
  ...
  await page.getByRole('button', { name: '生成交付包' }).click()
  await expect(page.getByText('交付五件套已生成：5 个文件')).toBeVisible({ timeout: 30000 })
  await expect(page.locator('strong', { hasText: `${projectName}_project_meta.md` })).toBeVisible()
  await expect(page.locator('strong', { hasText: `${projectName}_translation_prompt.txt` })).toBeVisible()
  await expect(page.locator('strong', { hasText: `${projectName}_glossary.xlsx` })).toBeVisible()
